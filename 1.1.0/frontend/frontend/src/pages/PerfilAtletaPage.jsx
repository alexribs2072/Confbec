...
function PerfilAtletaPage() {
  ...
  // Controle do Snackbar (Notificação de sucesso)
  ...
  useEffect(() => {
    const fetchPerfil = async () => {
      try {
        ...
        // Ignora 404 (perfil novo)
        if (err.response?.status !== 404) {
          setError('Erro ao carregar seus dados. Tente novamente.');
        }
      } finally {
        // Pequeno delay artificial para evitar "piscar" muito rápido se a API for instantânea
  ...
  const handleUploadFoto = async (e) => {
    ...
    fileData.append('foto', file); // Campo 'foto' conforme esperado pelo backend

    try {
      // Feedback visual imediato (opcional, mostra loading no botão se quiser)
      const res = await axios.post('/api/atletas/me/foto', fileData);
      setFormData(prev => ({ ...prev, foto_url: res.data.foto_url }));
      setToastOpen(true); // Sucesso
    } catch (err) {
  ...
  // --- Renderização com Skeleton (Carregamento Elegante) ---
  if (loading) {
    return (
      ...
      <Grid container spacing={4}>
        ...
        <Grid item xs={12} md={4}>
          ...
          >
            ...
            <Box sx={{ display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 1, color: 'success.main' }}>
              ...
              <Typography variant="caption" fontWeight="bold">CONTA ATIVA</Typography>
            </Box>
          </Paper>
        </Grid>
        ...
        <Grid item xs={12} md={8}>
          <Box component="form" onSubmit={handleSubmit}>
            ...
            <Box sx={{ mb: 3 }}>
              <Typography variant="h6" fontWeight="bold" gutterBottom sx={{ display: 'flex', alignItems: 'center', color: 'text.primary' }}>
                <PersonIcon color="primary" sx={{ mr: 1 }} /> Dados Pessoais
              </Typography>
            ...
            <Box sx={{ mb: 4 }}>
              <Typography variant="h6" fontWeight="bold" gutterBottom sx={{ display: 'flex', alignItems: 'center', color: 'text.primary' }}>
                <HomeIcon color="primary" sx={{ mr: 1 }} /> Endereço
              </Typography>
              <Paper variant="outlined" sx={{ p: 3, borderRadius: 2, bgcolor: 'white' }}>
                <Grid container spacing={3}>
