...
function PagamentoPage() {
  ...
  const context = pagamento?.context || (pagamento?.inscricaoCompeticaoId ? 'competicao' : 'filiacao');
  const titulo = context === 'competicao' ? 'Pague sua Inscrição na Competição' : 'Pague sua Taxa de Filiação';
  const descricao = pagamento?.descricao || (context === 'competicao' ? `Inscrição #${pagamento?.inscricaoCompeticaoId}` : 'Taxa de Filiação');
            ...
            {(hasPix || hasBoleto) && (
              <Box sx={{ width: '100%' }}>
                ...
                <Alert severity="info" sx={{ mt: 3, width: '100%' }}>
                  Após o pagamento, o status será atualizado automaticamente. Isso pode levar alguns minutos.
                </Alert>
              </Box>
            )}
          </>
