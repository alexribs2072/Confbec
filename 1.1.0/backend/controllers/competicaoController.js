...
async function getEventoOr404(eventoId, res) {
  const evento = await CompeticaoEvento.findByPk(eventoId, {
    include: [{ model: CompeticaoModalidade, as: 'modalidades' }]
  });
  if (!evento) {
...
exports.atualizarModalidadesDoEvento = async (req, res) => {
  try {
    ...
    const { competicao_modalidade_ids } = req.body || {};
    if (!Array.isArray(competicao_modalidade_ids)) {
      return res.status(400).json({ msg: 'Informe competicao_modalidade_ids como array.' });
    }

    const modalidades = await CompeticaoModalidade.findAll({
      where: { id: { [Op.in]: competicao_modalidade_ids } }
    });

    await evento.setModalidades(modalidades);
...
exports.criarInscricao = async (req, res) => {
  try {
    ...
    const taxa = Number(evento.taxa_inscricao || 0);
